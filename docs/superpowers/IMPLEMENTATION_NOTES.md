...
| OPEN | 시장 이벤트 공급자 | 현재는 검증된 JSON 이벤트만 읽는다. 자동 수집 공급자, 중복 제거, 공급 실패 경보가 필요하다. | 데이터 공급자 선정 시 |
| OPEN | 모니터링 보관·전달 | 현재 리포트는 표준 출력 JSON이다. SQLite 이력 저장과 Telegram 등 전달 채널은 미구현이다. | 운영 자동화 착수 시 |
...
- 보유종목의 1·5·20일 수익률, 거래량 급증, 추세, 손절, 순위를 매일 검토한다.
- 정규화된 기업·거시·지정학 이벤트를 종목에 연결하고 심각도와 신뢰도 기준을 적용한다.
- 일일 진입 후보는 현재 목표 종목 중 기준을 통과한 최대 3개만 보고한다.
- 주간 탐색은 전체 활성 유니버스에서 최대 5개를 찾되 기준 미달 종목으로 채우지 않는다.
- 신규 후보와 약한 보유종목의 점수 차이, 보유 허용 순위, 이벤트 위험을 함께 비교한다.
- 모든 리포트는 `execution_authorized=false`이며 주문 실행과 분리한다.
- 현재 입력은 JSON adapter 계약이며 실시간 뉴스·공시 공급자 연결은 별도 구현이 필요하다.
