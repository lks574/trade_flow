...
def _row(bar: DailyBar) -> tuple[object, ...]:
    return (
        bar.symbol,
...
class PriceRepository:
    ...
    def save_bars(self, bars: Sequence[DailyBar]) -> int:
        """Idempotent raw writer for historical backfill. Skips the snapshot
        quality gate, which validates 'safe to trade today', not bulk history."""
    ...
    def save_snapshot(self, snapshot: MarketDataSnapshot) -> int:
        ...
        rows = [_row(bar) for bar in snapshot.prices]
        with sqlite3.connect(self.database_path) as connection:
            before = connection.total_changes
            connection.executemany(_UPSERT, rows)
            changed = connection.total_changes - before
